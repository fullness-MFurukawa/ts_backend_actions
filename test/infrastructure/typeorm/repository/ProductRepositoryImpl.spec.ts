...
describe('ProductRepositoryImplの単体テスト', () => {
    ...
    let repository: ProductRepository<EntityManager>; // テストターゲット
    let dataSource: DataSource;// TypeORMのDataSource
    ...
    describe('findById()メソッド', () => {
        it("存在する商品Idを利用すると、商品を取得できる", async () => {
            const id = ProductId.fromString("8f81a72a-58ef-422b-b472-d982e8665292");
        ...
        it("存在しない商品Idを利用すると、nullが返される", async () => {
            const id = ProductId.fromString("8f81a72a-58ef-422b-b472-d982e8665293");
            const product = await repository.findById(id);
            // 取得結果を検証する
    ...
    describe("findByKeyword()メソッド", () => {
        it("キーワード'ボールペン'で検索した場合、対応する商品を取得できる", async () => {
            const keyword = ProductName.fromString("ボールペン");
        ...
        it("キーワードに一致する商品が存在しない場合、nullが返される", async () => {
            const keyword = ProductName.fromString("存在しない仕様品名");
            const products = await repository.findByKeyword(keyword);
    ...
    describe("exists()メソッド", () => {
        it("商品が存在する場合はtrueを返す", async () => {
            const productName = ProductName.fromString("水性ボールペン(黒)");
            const exists = await repository.exists(productName);
            expect(exists).toBe(true);
        });
        it("商品が存在しない場合はfalseを返す", async () => {
            const productName = ProductName.fromString("存在しない商品名");
            const exists = await repository.exists(productName);
    ...
    describe("create()メソッド", () => {
        it("新しい商品を登録できる", async () => {
            ...
            const product = Product.create(
                ProductName.fromString("新商品"),ProductPrice.fromNumber(300));
    ...
    describe("updateById()メソッド", () => {
        it("存在する商品の場合、商品名と商品単価を変更できtrueを返す", async () => {
            // 変更データを準備する
            const productId = ProductId.fromString("ac413f22-0cf1-490a-9635-7e9ca810e544"); 
            const updatedName = ProductName.fromString("更新後のボールペン");
            const updatedPrice = ProductPrice.fromNumber(150);
            const category = Category.fromExisting(
                CategoryId.fromString("b1524011-b6af-417e-8bf2-f449dd58b5c0"), 
                CategoryName.fromString("文房具" ));
            const product = Product.fromExisting(productId,updatedName,updatedPrice);
            product.changeCategory(category);
            // DataSourceからQueryRunnerを取得する
            const queryRunner = dataSource.createQueryRunner();
        ...
        it("存在しない商品の場合、商品名と商品単価を変更できずfalseを返す", async () => {
             // 変更データを準備する
             const productId = ProductId.fromString("ac413f22-0cf1-490a-9635-7e9ca810e545"); 
             const updatedName = ProductName.fromString("更新後のボールペン");
             const updatedPrice = ProductPrice.fromNumber(150);
             const category = Category.fromExisting(
                 CategoryId.fromString("b1524011-b6af-417e-8bf2-f449dd58b5c0"), 
    ...
    describe("deleteById()メソッド", () => {
        ...
        it("存在しない商品は削除できずfalseを返す", async () => {
            // 変更データを準備する
            const productId = ProductId.fromString("ac413f22-0cf1-490a-9635-7e9ca810e555");
            // DataSourceからQueryRunnerを取得する
            const queryRunner = dataSource.createQueryRunner();
            await queryRunner.connect();// データベース接続する
            await queryRunner.startTransaction();// トランザクションを開始する
