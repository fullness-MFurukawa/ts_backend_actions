import { ArgumentsHost, HttpStatus, InternalServerErrorException } from "@nestjs/common";
...
describe('HttpExceptionFilterのテスト', () => {
    let filter: HttpExceptionFilter;// テスト対象のフィルタ
    let mockResponse: any;          // モック化されたレスポンスオブジェクト
    let mockRequest: any;           // モック化されたリクエストオブジェクト
    let mockContext: ArgumentsHost; // モック化されたNestJSの実行コンテキスト
    ...
    it('DomainExceptionを処理してステータス400を返す', () => {
        const exception = new DomainException('ドメインエラー発生');
    ...
    it('InternalServerErrorExceptionを適切に処理している', () => {
        const exception = new InternalServerErrorException('内部エラー');
