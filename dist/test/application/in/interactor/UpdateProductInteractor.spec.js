...
describe('UpdateProductInteractorの単体テスト', () => {
    ...
    beforeAll(async () => {
        ...
        await app.init();
        // ユースケースを取得
        usecase = app.get('UpdateProductUsecase');
    });
    /**
     * すべてのテストの後処理
     */
    ...
    describe('()メソchangeッドのテスト', () => {
        ...
        it('存在しない商品を変更した場合、NotFoundExceptionがスローされる', async () => {
            ...
            await expect(usecase.change(updateProduct)).rejects.toThrow(NotFoundException_1.NotFoundException);
            await expect(usecase.change(updateProduct)).rejects.toThrow(`商品Id:(${updateProduct.id})の商品は存在しないため変更できませんでした。`);
        });
    });
});
